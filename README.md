...
- **Автоматизировал контроль качества кода**: настроил CI/CD (GitHub Actions)
  во всех пяти репозиториях — юнит-тесты (три репозитория с бизнес-логикой) и
  валидация DAG/воркфлоу/docker-compose (этот репозиторий и
  `n8n-business-automation`, где тестировать по сути нечего кроме
  конфигурации) прогоняются при каждом пуше, что предотвращает попадание
  регрессий в основную ветку без ручной проверки.
- **Добавил A/B-тестирование с честным power-анализом**
  (`product-marketing-analytics/ab_test/`): симулированный, но
  методологически настоящий тест на 200 клиентах — SRM-проверка,
  two-proportion z-test, power-анализ. Честный результат: заложенная
  разница в конверсии (6% против 11%) статистически не значима на этой
  выборке (p=0.205), а power-анализ прямо показывает почему — нужно
  почти в 5 раз больше клиентов для 80% мощности. Тот же принцип
  честности, что и у churn-модели с AUC≈0.5.
